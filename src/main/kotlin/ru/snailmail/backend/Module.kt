...
data class CreateLichkaRequest(val invitedId: UID) : Request
        ...
        authenticate {
            post("/createLichka") {
                requestData<CreateLichkaRequest>({ params, principal ->
                    ...
                    val fstUser = Master.findUserByLogin(principal.name) ?: throw IllegalArgumentException()
            ...
            post("/blockUser") {
                requestData<BlockOrUnblockUserRequest>({ params, principal ->
                    val user =
                        Master.findUserByLogin(principal.name) ?: throw IllegalArgumentException("User not found")
